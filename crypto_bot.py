...
# Введите ваш Binance API-ключ и секретный ключ
api_key = "binance api key"
secret_key = "binance api secret"

# Введите ваш токен бота и ID вашего канала
API_TOKEN = 'tg bot api token'
channel_id = -100xxxxxx7  # Замените на ваш ID канала
...
async def on_startup(dp):
    await send_message("Бот запущен. v1 (alpha beta)")

async def price_monitor():
    old_prices = get_binance_prices(crypto_pairs, api_key, secret_key)

    while True:
        await sleep(60)  # Изменение частоты обновления на 1 минуту
        new_prices = get_binance_prices(crypto_pairs, api_key, secret_key)

        for pair in crypto_pairs:
            token_name = pair.replace("USDT", "")  # Замените разделение по символу '/'
            old_price = old_prices[pair]
            new_price = new_prices[pair]
            if old_price == 0:  # Если old_price равно нулю, пропустите итерацию и перейдите к следующей паре
                continue
            ...
            if abs(percentage_difference) >= 5:
                ...
                message = f"{direction} #{token_name} | {action} | {abs(percentage_difference):.2f}% ({abs(price_difference):.2f}$) | Текущая цена монеты ${new_price:.2f}"
...
async def on_startup_and_monitor(dp):
    await on_startup(dp)
    await price_monitor()  # Добавьте круглые скобки здесь
